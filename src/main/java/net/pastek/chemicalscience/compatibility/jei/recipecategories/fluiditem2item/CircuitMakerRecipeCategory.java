...
public class CircuitMakerRecipeCategory extends FluidItem2ItemRecipeCategory<CircuitMakerRecipe> {
    ...
    public CircuitMakerRecipeCategory(IGuiHelper guiHelper) {
        super(guiHelper, CSTextUtils.jeiTranslated("circuit_maker_recipe", new Object[0]), INPUT_MACHINE, BACK_WRAP, RECIPE_TYPE, 50);
        this.setInputSlots(guiHelper, new ItemSlotObject[]{INPUT_SLOT1, INPUT_SLOT2, INPUT_SLOT3, INPUT_SLOT4, INPUT_SLOT5, INPUT_BUCKET_SLOT});
        this.setOutputSlots(guiHelper, new ItemSlotObject[]{OUTPUT_SLOT});
        this.setFluidInputs(guiHelper, new AbstractFluidGaugeObject[]{IN_GAUGE});
        this.setAnimatedArrows(guiHelper, new ArrowAnimatedObject[]{ANIM_RIGHT_ARROW});
        this.setLabels(new AbstractLabelWrapper[]{POWER_LABEL, TIME_LABEL});
    }

    static {
        INPUT_SLOT1 = new ItemSlotObject(SlotType.NORMAL, 28, 16, RecipeIngredientRole.INPUT);
        INPUT_SLOT2 = new ItemSlotObject(SlotType.NORMAL, 48, 16, RecipeIngredientRole.INPUT);
        INPUT_SLOT3 = new ItemSlotObject(SlotType.NORMAL, 48, 36, RecipeIngredientRole.INPUT);
        INPUT_SLOT4 = new ItemSlotObject(SlotType.NORMAL, 90, 36, RecipeIngredientRole.INPUT);
        INPUT_SLOT5 = new ItemSlotObject(SlotType.NORMAL, 110, 36, RecipeIngredientRole.INPUT);
        OUTPUT_SLOT = new ItemSlotObject(SlotType.NORMAL, 110, 16, RecipeIngredientRole.OUTPUT);
        INPUT_BUCKET_SLOT = new ItemSlotObject(SlotType.NORMAL, IconType.FLUID_DARK, 28, 36, RecipeIngredientRole.INPUT);
        ANIM_RIGHT_ARROW = new ArrowAnimatedObject(ProgressBars.PROGRESS_ARROW_RIGHT, 78, 16, StartDirection.LEFT);
